...
class PatientCostUtilityMonitor:
    ...
    def update(self, k, current_state, next_state):
        ...
        # update cost of state
            ...
            cost += 0.5 * (self._param.get_annual_state_costs(current_state) +
                            self._param.get_annual_state_costs(next_state)) * self._param.get_delta_t()

        # add cost of drug treatment
        if next_state in [P.HealthStats.POST_STROKE]:
            cost += self._param.get_annual_drug_cost() * self._param.get_delta_t()

        # update utility
        utility = 0.5 * (self._param.get_annual_state_utilities(current_state) +
                         self._param.get_annual_state_utilities(next_state)) * self._param.get_delta_t()
    ...
    def get_total_discounted_utility(self):
        return self._totalDiscountedUtil
